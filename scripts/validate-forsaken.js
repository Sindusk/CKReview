// Regression harness for lib/mechanics/ffxiv/dancingmad/forsaken.ts against
// FFLogs report folders in sampledata/ff/ (gitignored) produced by
// scripts/fetch-ff-report.js. Builds PlayerInfo[] the same way the real
// pipeline does (onlyLanded filter, debuffStatus mapping, sourceInstance/
// x/y plumbing), runs detectForsakenTowerErrors, and prints every error
// per pull — with REAL player names/classes resolved from each report
// folder's meta.json via getFFJobByName, not a hand-maintained JOBS table.
//
// Run from the repo root:  node scripts/validate-forsaken.js [reportDir]
//
...
// reference (ForsakenSuccess.json, ForsakenPull1Fail.json, etc.) were
