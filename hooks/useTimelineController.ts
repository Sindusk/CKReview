...
export default function useTimelineController({
  ...
  if (vod && pull && vod.isCalibrated && autoSeekKey !== autoSeekKeyRef.current) {
    ...
    console.log(`[timeline] auto-seek computed vodId=${vod.id} vodOffset=${vod.offset} pullId=${pull.id} pullStart=${pull.startTime} pullChanged=${pullChanged} withinPull=${withinPull} target=${target}`);
